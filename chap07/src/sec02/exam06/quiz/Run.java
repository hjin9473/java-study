...
//	일하는 중입니다.
	...
	public static void main(String[] args) {
		SchoolMember[] index = new SchoolMember[3];

		index[0] = new Student();
		index[1] = new Teacher();
		index[2] = new Staff();
		
		for (SchoolMember sm : index) {
            sm.introduce();

            if (sm instanceof Student student) {
                student.study();
            } else if (sm instanceof Teacher teacher) {
                teacher.teach();
            } else if (sm instanceof Staff staff) {
                staff.work();
            }
        }
